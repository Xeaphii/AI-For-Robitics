...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        a =  random.gauss(motion[angle_index], self.steering_noise)% (2. * pi)
        b = random.gauss(motion[motion_index], self.distance_noise)
        result = robot(self.length)
        result.set_noise(bearing_noise, steering_noise, distance_noise)
        beta = (b/float(self.length))*tan(a)
        if abs(beta) >=0.0001 : 
            ...
            result.x = cx + R*(sin(self.orientation+ beta)) # +random.gauss(0, self.distance_noise)
            result.y = cy - R*(cos(self.orientation+ beta))#+random.gauss(0, self.distance_noise)
        else:
            
            result.x = self.x + b*(cos(self.orientation))#+random.gauss(0, self.distance_noise)
            result.y = self.y + b*(sin(self.orientation))#+random.gauss(0, self.distance_noise)
        result.orientation = (self.orientation+beta)%(2*pi)#+random.gauss(0, self.steering_noise)
        # ADD CODE HERE
        #result = self
    ...
    # sense: 
    #    
    def sense(self,noise_switch = 1): #do not change the name of this function
        Z = []
        for landmark in landmarks:
            if noise_switch == 1:
                Z.append((atan2(landmark[0]-self.y,landmark[1]-self.x)-self.orientation+random.gauss(0, self.bearing_noise))%(2*pi))
            else:
